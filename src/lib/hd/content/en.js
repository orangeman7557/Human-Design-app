...
import { LABELS } from './labels-en.js';
...
const overrides = {
  ...
  // Brief second-person one-paragraph summary of each cross, for the report's
  // "Living your design / purpose" section (getCrossSummary). Keyed exactly like
  // crossEssence; no gate/quarter detail on purpose. Aug 2026.
  crossSummary: {
    'of the Sphinx': 'Your life gives others direction — not by telling them what to do, but by being so distinctly yourself that people orient themselves when they see you. Direction isn’t chased or pushed: it shows up on its own when you’re honest about what’s yours.',
    'of Laws': 'You have a nose for the order that makes things last — what holds a project, a family or a community together, and what will break it. Your theme is to order without stiffening, so what you build stands on its own.',
    'of Explanation': 'Yours is making understandable what isn’t yet: translating an idea that’s crystal-clear inside until it lands outside. It’s not about being right — it’s about finding the form, and the moment, in which others can receive it.',
    'of Consciousness': 'You turn what you’ve lived into understanding: you gather experiences, let them settle at your own pace and, in time, draw a clarity from them that others don’t see. Your awareness isn’t thought out, it’s distilled — and forcing it clouds it.',
    'of Eden': 'Your life is made by getting into experience, including the kind that hurts: contact, friction, the crisis that teaches what nothing else could. The key is timing — not every moment is a moment to step in.',
    'of Contagion': 'You move others by how you throw yourself into what’s yours, not by what you preach: when something truly catches fire in you, the enthusiasm spreads. Your care is in what you say yes to, because a yes given in the heat of the moment can cost years.',
    'of Planning': 'You see the steps that lead from here to there and share them out, putting structure at the service of something shared. Your delicate point is agreements and rest: a deal accepted reluctantly, or a lack of pause, turns the plan into a burden.',
    'of the Vessel of Love': 'You’re the place where others feel accepted without having to justify themselves — and that happens because of who you are, not what you do. The condition is that love starts inside: an empty vessel holds no one.',
    'of Service': 'You’re built to improve what’s there: you see the flaw, yes, but in the service of making things work better for everyone. The edge is that the same eye that perfects can turn into endless criticism; if correcting stops bringing joy, something has gone wrong.',
    'of the Four Ways': 'You move forward by alternating: you come close and pull back, join the group and step out to process it. It isn’t inconsistency, it’s your method — you need the retreat for the approach to have something to offer.',
    'of the Sleeping Phoenix': 'You hold an enormous energy that isn’t spent in a steady stream but waits and, when the moment comes, unfolds all at once. Mistaking that for slowness is a misreading: your life changes colour with your mood, and there’s the key — don’t decide from the low point or promise from the high.',
    'of Tension': 'You carry a background dissatisfaction that isn’t a flaw: it’s what pushes you to go deeper, to stand your ground and to refuse the superficial. It all depends on where you discharge it — against your own life it exhausts; in the service of something worthwhile, it’s one of your most productive forces.',
    'of Rulership': 'Yours is administering what’s shared: the authority that appears when someone gathers, orders and distributes well, and does it with a grace that makes it readily accepted. The line with manipulation is played out in every small decision.',
    'of the Unexpected': 'Your life doesn’t follow the expected line: it arrives through turns no one planned, and the good is in them. Lived defensively it feels like instability; accepted, it makes you rich in experience and able to care for others because of what you’ve been through.',
    'of Maya': 'You move within the illusion of the material world without quite believing it: a good eye for the practical — what lasts, what ends — and, at the same time, a background question no data answers. It’s crossed using both hands, staying neither only in the detail nor only in the mystery.',
    'of Penetration': 'You produce in others a jolt that breaks their routine and makes them see something they didn’t — often without meaning to, just by how you enter. The delicate part is the when and the who: heard in time, the shock is an awakening; mistimed, it’s a collision.',
    'of Defiance': 'What’s yours asserts itself against others and with others: you often find yourself disagreeing — not for fun but because you see that something doesn’t fit — and that disagreement ends up useful to the group. Defying out of habit, though, is just noise.',
    'of Wishes': 'What you long for doesn’t stay inside: it becomes something that moves others, an impulse that gets things going in the collective. Careful not to mistake the fantasy for the plan — it’s the fuel, not the route.',
    'of Revolution': 'You change the rules of the deal when an agreement stops being fair: you don’t tear down for its own sake, you reformulate. It works when you have something to offer in exchange; rejecting without a proposal exhausts everyone, starting with you.',
    'of Separation': 'Your gift is to stand apart, not to merge with the group: you bring a view that hasn’t caught the mood of the room. The cost is feeling separate; knowing when to open up turns separation into perspective rather than isolation.',
    'of the Plane': 'Your theme is raising the level of relationships: taking contact with others to something deeper than the surface. It plays out in timing — forcing intimacy when it isn’t time lowers the plane instead of raising it.',
    'of Masks': 'You switch roles easily depending on the setting, and precisely because of that you reach very different people. The masks aren’t lies; the risk is losing track of which one is yours — a mask works as long as you know it’s a mask.',
    'of Uncertainty': 'You live without fixed certainties about your mood or your course, and that openness is exactly what lets you create something genuinely new with others. The trap is demanding an emotional steadiness that isn’t yours to have.',
    'of Identification': 'You have an eye for spotting, amid the collective noise, what will actually lead somewhere. The wear comes when you identify with what you point out: seeing the pattern doesn’t oblige you to carry it.',
    'of Prevention': 'You catch the flaw in a pattern while it’s still small, and your contribution is to warn in time. It’s only well received when asked for: correcting unasked turns prevention into reproach.',
    'of Education': 'Yours is to transmit in a way that changes the other — not to lecture. Your ideas only educate when they find the right moment; said too soon, the best of them sounds like a sermon. Your talent is in the when, not the what.',
    'of Upheaval': 'You stir what was comfortable, not for fun but because you see a structure that won’t hold and can’t stay quiet. You’re one of the energies that push a group to change; the difference between shaking things up and just bothering is whether there’s something better behind it.',
    'of Refinement': 'You polish what already exists until it truly serves: you sense what people are missing and process it alone, and from that comes a very fine sensitivity. You need the retreat — without the pause, that sensitivity turns into overload.',
    'of Duality': 'You’re fulfilled in relationship, in pairing up — with a person, a partner, a cause: what you can’t do alone, the pair can. And the pair is sustained by rest; a bond with no withdrawal ends in resentment.',
    'of Endeavor': 'You carry the energy of climbing: ambition put to building something with others, brick by brick, to reach higher than where you started. It needs to be recognised to thrive — pushing without an alliance is scaling alone a wall that belonged to everyone.',
    'of Informing': 'Yours is to bring the other up to date: a social grace in the service of people knowing what they need to know. It sounds minor and it’s one of the great preventers of friction — on the condition of the right moment, because informing at the wrong time is as useless as not informing.',
    'of Dedication': 'You don’t spread yourself thin: you choose and pour yourself in all the way, and that devotion is what others end up following. Since you say yes easily, choosing well is everything — devoting yourself to the wrong thing costs years.',
    'of Incarnation': 'You bring something into the world through your own life: what you’ve chewed over and remembered ends up taking shape in the role you hold for others. It takes patience with repetition — you return to the same point until, one day, you bring back something different.',
    'of Healing': 'You heal by returning someone to their own shape, not by fixing them, and you do it mostly through how you are, not what you do. Your challenge is to love without conditions or preferences, and that includes whoever doesn’t return it.',
    'of Confrontation': 'You open the hard conversation a group needs and no one wants to start: you put in front what was being avoided. It’s not a fight; the edge is honesty — an honest confrontation changes something, a manipulated one only wins the argument.',
    'of Alignment': 'You put care where it truly matters: you weigh what’s worth it and give it your energy, instead of spreading it blindly. To care for everything is to care for nothing — you’re here to choose, even when choosing hurts.',
    'of Industry': 'Your gift is to sustain a yes over time: you get in and hold on, and with that you carry through projects others abandon. It all depends on what you said yes to — a commitment sustained on the wrong thing is the very definition of burnout.',
    'of the Alpha': 'You go ahead not by imposing yourself, but because your voice names what the group didn’t yet know how to say. It’s a leadership that’s granted, not taken — to lead with no one truly following is talking to yourself.',
    'left:of Limitation': 'For you, accepting the frame is the good news: it’s what lets something be finished and last. You don’t cut for its own sake, you cut so there can be form — what’s stretched past the limit doesn’t grow, it breaks.',
    'of Migration': 'You take your people somewhere better — a change of job, of city, of way of doing things — and hold the bond together through the move. Your life theme is the tension between leaving for something new and keeping the pact.',
    'of Individualism': 'You defend the right to be different by standing your ground, and in holding your own strangeness you make room for others’. Your fight needs a cause: without something worthwhile, it turns into fighting with everything.',
    'of Cycles': 'You live beginnings and endings as your material: it’s not about sustaining one single thing forever, but about taking each cycle to its close before opening the next. What hurts is leaving things half done.',
    'of Spirit': 'You take mood as a serious matter: you explore emotional freedom and carry it into your relationships. There’s nothing to fix — the melancholy isn’t a flaw; deciding from it, though, is.',
    'of Distraction': 'You have the art of pulling someone out of where they were stuck: a story, a detour, a sideways idea, and suddenly they see something else. Without a destination, distraction turns into scatter — a good detour leads somewhere.',
    'of the Clarion': 'Yours is to say something that wakes people up, not to say a lot: a sound that cuts through the noise and makes people look up. It’s a call with a single attempt — repeating it turns it into noise.',
    'of Demands': 'You ask — of others and of yourself — and you’re usually within your rights, because you see what’s truly missing. Well placed, it’s the energy that raises a group’s bar; badly placed, it exhausts everyone — demand from calm, not from haste.',
    'of Obscuration': 'You live with a background question no data resolves, and your contribution is to hold it up rather than cover it over with explanations. Sometimes the honest thing is to say it’s still dark.',
    'of Dominion': 'Your role is to take charge: you put doubt in the service of checking things before others carry the cost of a mistake. It’s responsibility, not power — and that doubt is made to review what’s outside, not to distrust yourself.',
    'of Self-expression': 'You have a fixed, non-negotiable role: to say what’s yours. It isn’t a calling you choose, it’s a need — silencing what’s yours doesn’t save you conflict, it only postpones it.',
    'of the Driver': 'Your role is to steer, not to push: to be where you need to be so others find their course. It’s quiet and fixed — seeking recognition for it tends to spoil it.',
    'of Mutation': 'You bring what didn’t exist yet, with all its initial mess. Your role is to hold that chaos long enough for it to set, without rushing it or abandoning it.',
    'of Formulization': 'You give a loose idea a closed form: the formula works or it doesn’t, with that precision. The temptation is to formulate before you have the whole answer.',
    'of Habits': 'Your habits are the scaffolding of your life: your role is to hold your own rhythm without being pulled along by anyone else’s. Breaking it to please others costs dearly.',
    'of Conflict': 'For you, conflict is the work, not the accident: to be in the friction without fleeing or letting it fester. The edge is in timing — not every moment is a moment to step in.',
    'of Interaction': 'Your role is to be among people, taking in and giving back: it’s not decorative sociability, it’s your function. Isolation here switches off your whole chart.',
    'of Contribution': 'You put what’s yours in the service of something without dissolving into it: you contribute without asking permission and without needing applause.',
    'of Focus': 'You hold attention where others lose it: it’s a role of mental endurance. Your wear comes when you focus on what didn’t deserve it.',
    'of Behavior': 'Your role is to behave as you are, without adjusting. It sounds easy and it’s one of the most demanding — any extra adaptation is paid for inside.',
    'of Ideas': 'Having ideas is, for you, a condition, not an achievement: your role isn’t to carry them all out, but to offer them to whoever can.',
    'of Articulation': 'You find the exact form and the exact moment to say something. Your role lives in the wait — said at the wrong time, it can lose all its value.',
    'of Listening': 'People tell you things without knowing why: listening is your craft. It’s spoiled if you use what you’ve heard for anything other than giving it back well.',
    'of Empowering': 'You put resources where someone can use them better than you: you’re a quiet patron. The edge is giving them to someone who won’t move them.',
    'of Extremes': 'You inhabit the extremes without normalising: your odd rhythms are the contribution — you widen what counts as human.',
    'of Experimentation': 'Your role is to try before knowing: to jump in, with error as part of the method. But without repeating, the leap stays a leap with no skill.',
    'of Opinions': 'Having opinions is your role, not a vice. The delicate part is that an opinion isn’t a truth: it helps when offered and gets in the way when imposed.',
    'of Correction': 'You see what could be better before anyone else: correcting is your fixed craft. Well aimed, it’s a gift to the group; aimed at yourself without brakes, it’s corrosive.',
    'of Need': 'You notice the need before it’s spoken: you’re very sensitive and very useful, and you also wear out if you don’t respect your retreat.',
    'of the Now': 'Being in the now is your whole function: neither over-planning nor chewing over the past. Your independence protects that present from other people’s agendas.',
    'of Control': 'Your role is to manage what’s yours without anyone managing it for you. Extended to what belongs to others, the same impulse turns into a small tyranny.',
    'of Grace': 'Grace is your gift: knowing how to be present, to listen, to open up, to fall silent at the right moment. It’s a very fine social role, and it depends on the mood of the day.',
    'of Assimilation': 'You make the strange digestible: you translate the singular into something understandable. It plays out in timing — the same phrase is brilliant or incomprehensible depending on when you drop it.',
    'of Rationalization': 'You make sense of what hurt: it’s a productive chewing-over. The edge is when the rumination stays in a loop and never reaches a conclusion.',
    'of Innocence': 'Your role is to love without conditions or preferences, and that includes whoever doesn’t deserve it. It’s not naivety: it’s a sustained choice, and a hard one.',
    'of the Trickster': 'You sell, and someone needs to sell: your role is legitimate as long as what’s sold is real. The same talent, with the truth stretched, becomes manipulation.',
    'of Caring': 'Caring is your fixed function, and the warning is old: you care for others easily and for yourself with difficulty. There’s the whole of your learning.',
    'of Risks': 'Taking risks is your role: without the risk, the meaning doesn’t appear. What doesn’t count is risk for the adrenaline — you always ask what for.',
    'of Commitment': 'Committing is your entire function: a commitment isn’t abandoned. That’s why the yes matters so much — a yes badly given mortgages years.',
    'of Fates': 'Your desire ends up building your fate: what you truly long for eventually happens, for better and for worse. It’s an intense role.',
    'of Influence': 'Influencing is your fixed role, with one condition: it only works if there’s someone who wants to follow you. Influence taken by force isn’t influence.',
    'of Conservation': 'You protect what deserves to survive: you’re prudent and necessary. Your shadow is fear of change dressed up as good sense.',
    'of Retreat': 'Withdrawing is a function, not a flight: your material cooks in the silence, and what comes out of it is what you have to tell.',
    'of Power': 'Your power isn’t explained, it’s used: it’s available force. The trap is spending it on what responds to nothing.',
    'of Experience': 'Experiencing life is your role: the contribution comes afterwards, when you tell what you’ve lived in many places. Without rhythm, experience turns into flight.',
    'of Crisis': 'For you, crisis is the terrain, not the exception: your role is to go through them without dramatising, and not to step into the next before digesting the last.',
    'of Bargains': 'The pact is your function: giving and receiving on clear terms. And its rule is hard — a deal you accept unwillingly eventually collects its due.',
    'of Opposition': 'Opposing is your role, and it isn’t negativity: without someone to stand up, a lot of bad things pass unnoticed. The condition is that the cause truly exists.',
    'of Provocation': 'Provoking is uncovering: an uncomfortable question and, suddenly, out comes what was being kept quiet. Useful when you’re after something; cruel when you’re only after a reaction.',
    'of Denial': 'Refusing is a capacity, not a flaw: to say no and withdraw to recover. It breaks down when the withdrawal turns into permanent isolation.',
    'of Fantasy': 'Your fantasy is the spark of everything that later exists: you’re imaginative. Its limit is real — you imagine far more than one life can hold.',
    'of Completion': 'Completing is your whole role: few people finish, you do. Your wear comes when you get into cycles that should never have started.',
    'of Insight': 'Your intuition arrives whole and without explanation: your role is to hold it until you find how to say it. Blurting it out raw usually sounds absurd.',
    'of Alertness': 'Being alert is your function: you spot the pattern that already went wrong before. It’s an instinct, not a reasoning, and it works best when you don’t argue it.',
    'of Possession': 'Your role is to take charge of what’s shared, not to hoard: you keep the group’s resources. And the line with appropriation is thin.',
    'of Serendipity': 'You’re where you need to be without having planned it: it’s a fortunate role, with a physical condition — caring for your body is, here, caring for your luck.',
    'of Oppression': 'You feel you can’t find the way out while you process: your role is to bear that feeling without forcing the conclusion. Understanding comes on its own, and it comes late.',
    'of Depth': 'Depth is your role, and it comes with its fear attached: the sense of not knowing enough. That fear is your engine, not a sign that it’s true.',
    'of Principles': 'Having principles is, for you, a social function: someone has to mark where the line is. Your edge is rigidity — a principle that’s never reviewed turns into dogma.',
    'of Values': 'Keeping the values is your fixed role: it’s not moralism, it’s noticing when a group is skipping what kept it standing, and saying so.',
    'of Shock': 'Your role is the jolt that snaps someone out of autopilot: you tend to cause it without meaning to, just by how you enter.',
    'of Stillness': 'Stillness is, for you, an active function, not passivity: you stay long enough for what can’t be seen in motion to become visible. It’s hard in a world that rewards haste.',
    'of Beginnings': 'Beginning is your role, and it doesn’t include finishing. Understanding that lifts a lot of guilt: your contribution is the start, and others can take it to the end.',
    'of Ambition': 'Ambition is your function, with one rule: it thrives when it’s recognised. Pushing without an alliance leaves you climbing a very long ladder alone.',
    'of Moods': 'Your moods are your working ground, not a problem to solve: your role is to live them honestly. The one practical rule is not to decide from the low point.',
    'of Stimulation': 'Your role is the storyteller’s: to keep people awake with what you tell. Your limit is real — without it, the stimulation turns into noise.',
    'of Intuition': 'Your intuition is your whole instrument: it works in the present and only once, and reasoning it out afterwards erases it. Your role is to trust that first prompt.',
    'of Vitality': 'Vitality is your role, and it isn’t optimism: it’s a physical energy that pushes you to improve things because being alive feels good. When it disappears, something is being over-corrected.',
    'of Strategy': 'Yours is knowing how to approach: how you cross the distance to another person. It’s an intimate, very concrete role — forcing it closes exactly what it meant to open.',
    'juxtaposition:of Limitation': 'The limit is your function, and it’s good news: without a frame there’s no form. Its shadow is the melancholy of what doesn’t fit — real, and not a reason to force the frame.',
    'of Thinking': 'Thinking is, for you, a pressure, not a pastime: it pushes you to know without guaranteeing an answer. Your role is to hold the question without manufacturing conclusions to quiet it.',
    'of Detail': 'Your role is to name things so they can be handled: an enormous, little-recognised contribution. Your shadow is getting lost in the small and missing the whole.',
    'of Doubts': 'Doubting is your function, and it’s valuable: someone has to ask whether this really works. Aimed inward it paralyses; aimed outward it protects the group.',
    'of Confusion': 'For you, confusion is the normal working state, not a fault: you take in more material than you can order, and the order arrives on its own, in time. Forcing it is the only thing that makes it worse.'
  },
