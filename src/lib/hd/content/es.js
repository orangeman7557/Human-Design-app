...
export default {
  ...
  // "Tu propósito" section (getCrossSummary). Keyed exactly like crossEssence;
  // no gate/quarter detail on purpose — that lives in the cross drawer. Aug 2026.
  crossSummary: {
    'of the Sphinx': 'Tu vida da dirección a los demás no diciéndoles qué hacer, sino siendo tú mismo de una forma tan definida que el resto se orienta al verte. La orientación no se busca ni se empuja: aparece sola cuando eres honesto con lo tuyo.',
    'of Laws': 'Tienes olfato para el orden que hace que las cosas duren: lo que sostiene un proyecto, una familia o una comunidad, y lo que las va a romper. Tu tema es ordenar sin agarrotar, para que lo que construyes se aguante solo.',
    'of Explanation': 'Lo tuyo es hacer comprensible lo que aún no lo es: traducir una idea que dentro se ve clarísima hasta que fuera se entiende. No va de tener razón, sino de encontrar la forma —y el momento— en que los demás puedan recibirla.',
    'of Consciousness': 'Conviertes lo vivido en comprensión: acumulas experiencias, las dejas reposar a tu ritmo y, con tiempo, sacas de ahí una claridad que otros no ven. Tu consciencia no se piensa, se destila —y forzarla la enturbia.',
    'of Eden': 'Tu vida se hace metiéndote en la experiencia, incluida la que duele: el contacto, el roce, la crisis que enseña lo que no se aprendía de otro modo. La clave es el ritmo —no todo momento es momento de entrar.',
    'of Contagion': 'Mueves a otros por cómo te entregas a lo tuyo, no por lo que predicas: cuando algo te prende de verdad, el entusiasmo se pega. Tu cuidado está en a qué le dices que sí, porque un sí dado en caliente puede costar años.',
    'of Planning': 'Ves los pasos que llevan de aquí a allí y los repartes, poniendo estructura al servicio de algo compartido. Tu punto delicado son los acuerdos y el descanso: un pacto aceptado sin ganas, o la falta de pausa, convierten el plan en carga.',
    'of the Vessel of Love': 'Eres el sitio donde otros se sienten aceptados sin tener que justificarse —y eso ocurre por lo que eres, no por lo que haces. La condición es que el amor empiece por dentro: un receptáculo vacío no sostiene a nadie.',
    'of Service': 'Estás hecho para mejorar lo que hay: ves el fallo, sí, pero al servicio de que la cosa funcione mejor para todos. El filo es que la misma mirada que perfecciona puede volverse crítica sin freno; si corregir deja de dar alegría, algo se ha torcido.',
    'of the Four Ways': 'Avanzas alternando: te acercas y te retiras, entras en el grupo y sales a procesarlo. No es incoherencia, es tu método —necesitas el repliegue para que el acercamiento tenga algo que aportar.',
    'of the Sleeping Phoenix': 'Guardas una energía enorme que no se gasta a chorro continuo, sino que espera y, cuando llega el momento, se despliega entera. Confundir eso con lentitud es un error: tu vida cambia de color con tu ánimo, y ahí está la clave —no decidir desde el punto bajo ni prometer desde el alto.',
    'of Tension': 'Llevas una insatisfacción de fondo que no es un defecto: es lo que te empuja a profundizar, a plantar cara y a no conformarte con lo superficial. Todo depende de dónde la descargues —contra tu propia vida agota; al servicio de algo que valga la pena, es de tus fuerzas más productivas.',
    'of Rulership': 'Lo tuyo es administrar lo común: la autoridad que aparece cuando alguien reúne, ordena y reparte bien, y lo hace con una gracia que hace que se acepte de buen grado. La línea con la manipulación se juega en cada pequeña decisión.',
    'of the Unexpected': 'Tu vida no sigue la línea prevista: llega por giros que nadie había planeado, y en ellos está lo bueno. Vivido a la defensiva se siente como inestabilidad; aceptado, te vuelve rico en experiencia y capaz de cuidar a otros por lo vivido.',
    'of Maya': 'Te mueves dentro de la ilusión del mundo material sin creértela del todo: buen ojo para lo práctico —lo que dura, lo que se termina— y, a la vez, una pregunta de fondo que ningún dato responde. Se atraviesa usando las dos manos, sin quedarte solo en el detalle ni solo en el misterio.',
    'of Penetration': 'Produces en otros un sobresalto que rompe su rutina y les hace ver algo que no veían —a menudo sin pretenderlo, solo por cómo entras. Lo delicado es el cuándo y a quién: escuchado a tiempo, el golpe es despertar; a destiempo, es choque.',
    'of Defiance': 'Lo tuyo se afirma frente a otros y con otros: sueles encontrarte discrepando, no por gusto sino porque ves que algo no encaja, y esa discrepancia acaba siendo útil para el grupo. Desafiar por costumbre, en cambio, es solo ruido.',
    'of Wishes': 'Lo que anhelas no se queda dentro: se convierte en algo que mueve a otros, un impulso que arranca cosas en lo colectivo. Cuidado con confundir la fantasía con el plan —es el combustible, no la ruta.',
    'of Revolution': 'Cambias las reglas del trato cuando un acuerdo deja de ser justo: no derribas por derribar, reformulas. Funciona cuando tienes algo que ofrecer a cambio; rechazar sin propuesta agota a todos, empezando por ti.',
    'of Separation': 'Tu don es distinguirte, no fundirte con el grupo: aportas una mirada que no se ha contagiado del ambiente. El coste es sentirte aparte; sabiendo cuándo abrirte, la separación se vuelve perspectiva y no aislamiento.',
    'of the Plane': 'Tu tema es subir el nivel de las relaciones: llevar el contacto con otros a algo más hondo que la superficie. Se juega en el ritmo —forzar intimidad cuando no toca baja el plano en vez de subirlo.',
    'of Masks': 'Cambias de papel con soltura según el sitio, y precisamente por eso llegas a gente muy distinta. Las máscaras no son mentira; el riesgo es perder el hilo de cuál es la tuya —una máscara sirve mientras sabes que lo es.',
    'of Uncertainty': 'Vives sin certezas fijas sobre tu ánimo ni tu rumbo, y esa apertura es justo lo que te deja crear algo genuinamente nuevo con otros. La trampa es exigirte una constancia emocional que no te toca.',
    'of Identification': 'Tienes ojo para reconocer, dentro del ruido colectivo, aquello que de verdad va a llevar a alguna parte. El desgaste llega cuando te identificas con lo que señalas: ver el patrón no te obliga a cargar con él.',
    'of Prevention': 'Detectas el fallo de un patrón cuando aún es pequeño, y tu aportación es avisar a tiempo. Solo se recibe bien si te lo piden: corregir sin que nadie lo pida convierte la prevención en reproche.',
    'of Education': 'Lo tuyo es transmitir de forma que el otro cambie —no dar clase. Tus ideas solo educan cuando encuentran el momento; dichas a destiempo, la mejor suena a sermón. Tu talento está en el cuándo, no en el qué.',
    'of Upheaval': 'Remueves lo que estaba cómodo, no por gusto sino porque ves una estructura que no aguanta y no puedes callarte. Eres de las energías que empujan a un grupo a cambiar; la diferencia entre sacudir y molestar está en si hay algo mejor detrás.',
    'of Refinement': 'Pules lo que ya existe hasta que sirva de verdad: percibes lo que a la gente le falta y lo procesas a solas, y de ahí sale una sensibilidad muy fina. Necesitas el retiro —sin la pausa, esa sensibilidad se te vuelve sobrecarga.',
    'of Duality': 'Te cumples en la relación, en el emparejarte —con una persona, un socio, una causa—: lo que solo no puedes, el par sí. Y el par se sostiene con descanso; un pacto sin retirada acaba en resentimiento.',
    'of Endeavor': 'Llevas la energía de subir: ambición puesta a construir algo con otros, ladrillo a ladrillo, para llegar más alto de donde partiste. Necesita ser reconocida para prosperar —empujar sin alianza es escalar en solitario una pared que era de todos.',
    'of Informing': 'Lo tuyo es poner al día al otro: una gracia social al servicio de que la gente sepa lo que necesita saber. Suena menor y es de lo que más fricción evita —con la condición del momento justo, porque informar a destiempo es tan inútil como no informar.',
    'of Dedication': 'No repartes: eliges y te vuelcas hasta el fondo, y esa entrega es lo que otros acaban siguiendo. Como dices que sí con facilidad, elegir bien lo es todo —dedicarte a lo equivocado cuesta años.',
    'of Incarnation': 'Traes algo al mundo a través de tu propia vida: lo que has rumiado y recordado acaba tomando forma en el papel que ocupas ante otros. Requiere paciencia con la repetición —vuelves al mismo punto hasta que un día traes algo distinto.',
    'of Healing': 'Sanas devolviendo a alguien a su propia forma, no arreglándolo, y lo haces sobre todo por cómo estás, no por lo que haces. Tu reto es amar sin condiciones ni preferencias, y eso incluye a quien no te lo devuelve.',
    'of Confrontation': 'Abres la conversación difícil que un grupo necesita y nadie quiere empezar: pones delante lo que se evitaba. No es pelea; el filo es la honestidad —una confrontación honesta cambia algo, una manipulada solo gana la discusión.',
    'of Alignment': 'Pones el cuidado donde de verdad importa: mides qué merece la pena y le dedicas tu energía, en vez de repartirla a ciegas. Cuidar de todo es no cuidar de nada —estás aquí para elegir, aunque elegir duela.',
    'of Industry': 'Tu don es sostener un sí en el tiempo: te metes y aguantas, y con eso sacas adelante proyectos que otros abandonan. Todo depende de a qué le dijiste que sí —un compromiso sostenido en lo equivocado es la definición del desgaste.',
    'of the Alpha': 'Vas delante no por imponerte, sino porque tu voz nombra lo que el grupo aún no sabía decir. Es un liderazgo que se otorga, no se toma —liderar sin que nadie te siga de verdad es hablar solo.',
    'left:of Limitation': 'Para ti, aceptar el marco es la buena noticia: es lo que permite que algo se termine y dure. No recortas por gusto, recortas para que haya forma —lo que se estira más allá del límite no crece, se rompe.',
    'of Migration': 'Llevas a los tuyos a un sitio mejor —un cambio de trabajo, de ciudad, de manera de hacer las cosas— y sostienes el vínculo durante el traslado. Tu tema de vida es la tensión entre irte a por algo nuevo y sostener el pacto.',
    'of Individualism': 'Defiendes el derecho a ser diferente a base de plantar cara, y al sostener tu propia rareza abres sitio para la de otros. Tu pelea necesita causa: sin algo que merezca la pena, se convierte en pelearte con todo.',
    'of Cycles': 'Vives el empezar y el terminar como tu materia: no se trata de sostener una sola cosa para siempre, sino de llevar cada ciclo hasta su final antes de abrir el siguiente. Lo que duele es dejar cosas a medias.',
    'of Spirit': 'Te tomas el estado de ánimo como asunto serio: exploras la libertad emocional y la llevas a tus relaciones. No hay nada que arreglar —la melancolía no es un fallo; decidir desde ella, sí.',
    'of Distraction': 'Tienes el arte de sacar a alguien de donde estaba metido: una historia, un desvío, una idea lateral, y de pronto ve otra cosa. Sin un destino, la distracción se vuelve dispersión —un buen desvío lleva a algún sitio.',
    'of the Clarion': 'Lo tuyo es decir algo que despierta, no decir mucho: un sonido que atraviesa el ruido y hace que la gente levante la cabeza. Es una llamada de un solo intento —repetirla la convierte en ruido.',
    'of Demands': 'Pides, a los demás y a ti mismo, y sueles estar en tu derecho porque ves lo que falta de verdad. Bien puesta, es la energía que sube el listón de un grupo; mal puesta, agota a todos —exige desde la calma, no desde la prisa.',
    'of Obscuration': 'Vives con una pregunta de fondo que ningún dato resuelve, y tu aportación es sostenerla en alto en vez de taparla con explicaciones. A veces lo honesto es decir que todavía está oscuro.',
    'of Dominion': 'Tu papel es hacerte cargo: pones la duda al servicio de comprobar las cosas antes de que otros carguen con un error. Es responsabilidad, no poder —y esa duda está hecha para revisar lo de fuera, no para desconfiar de ti mismo.',
    'of Self-expression': 'Tienes un papel fijo y no negociable: decir lo tuyo. No es una vocación que elijas, es una necesidad —callar lo propio no te ahorra conflictos, solo los aplaza.',
    'of the Driver': 'Tu papel es orientar, no empujar: estar donde hay que estar para que otros encuentren su rumbo. Es silencioso y fijo —buscar reconocimiento por ello suele estropearlo.',
    'of Mutation': 'Traes lo que aún no existía, con todo su desorden inicial. Tu papel es aguantar ese caos el tiempo suficiente para que cuaje, sin apresurarlo ni abandonarlo.',
    'of Formulization': 'Le das forma cerrada a una idea suelta: la fórmula sirve o no sirve, con esa precisión. La tentación es formular antes de tener la respuesta entera.',
    'of Habits': 'Tus hábitos son el andamiaje de tu vida: tu papel es sostener un ritmo propio sin dejarte arrastrar por el de nadie. Romperlo por complacer sale caro.',
    'of Conflict': 'Para ti el conflicto es el trabajo, no el accidente: estar en el roce sin huir ni enquistarte. El filo está en el ritmo —no todo momento es momento de entrar.',
    'of Interaction': 'Tu papel es estar entre la gente, recogiendo y devolviendo: no es sociabilidad decorativa, es tu función. El aislamiento aquí apaga tu carta entera.',
    'of Contribution': 'Pones lo tuyo al servicio de algo sin diluirte en ello: aportas sin pedir permiso y sin necesitar aplauso.',
    'of Focus': 'Sostienes la atención donde otros la pierden: es un papel de resistencia mental. Tu desgaste llega cuando te enfocas en lo que no lo merecía.',
    'of Behavior': 'Tu papel es comportarte como eres, sin ajustarte. Suena fácil y es de lo más exigente —cualquier adaptación de más se paga por dentro.',
    'of Ideas': 'Tener ideas es en ti una condición, no un logro: tu papel no es realizarlas todas, sino ofrecerlas a quien sí puede.',
    'of Articulation': 'Encuentras la forma exacta y el momento exacto de decir algo. Tu papel vive en la espera —dicho a destiempo, lo mismo pierde todo su valor.',
    'of Listening': 'La gente te cuenta cosas sin saber por qué: escuchar es tu oficio. Se estropea si usas lo escuchado para otra cosa que no sea devolverlo bien.',
    'of Empowering': 'Pones recursos donde alguien puede usarlos mejor que tú: eres un mecenas silencioso. El filo está en dárselos a quien no los va a mover.',
    'of Extremes': 'Habitas los extremos sin normalizarte: tus ritmos raros son la aportación —ensanchas lo que se considera humano.',
    'of Experimentation': 'Tu papel es probar antes de saber: lanzarte, con el error como parte del método. Pero sin repetir, el salto se queda en un salto sin destreza.',
    'of Opinions': 'Opinar es tu papel, no un vicio. Lo delicado es que una opinión no es una verdad: sirve cuando la ofreces y estorba cuando la impones.',
    'of Correction': 'Ves lo mejorable antes que nadie: corregir es tu oficio fijo. Bien dirigido es un regalo para el grupo; dirigido a ti mismo sin freno, es corrosivo.',
    'of Need': 'Notas la necesidad antes de que se diga: eres muy sensible y muy útil, y también te agotas si no respetas tu retiro.',
    'of the Now': 'Estar en el ahora es tu función completa: ni planificar de más ni rumiar el pasado. Tu independencia protege ese presente de las agendas ajenas.',
    'of Control': 'Tu papel es administrar lo tuyo sin que nadie lo administre por ti. Extendido a lo ajeno, el mismo impulso se vuelve una pequeña tiranía.',
    'of Grace': 'La gracia es tu don: saber estar, escuchar, abrirte, callar en el momento justo. Es un papel social muy fino, y depende del estado de ánimo del día.',
    'of Assimilation': 'Haces digerible lo raro: traduces lo singular a algo comprensible. Se juega en el momento —la misma frase es genial o incomprensible según cuándo la sueltes.',
    'of Rationalization': 'Le das sentido a lo que dolió: es una rumia productiva. El filo es cuando la rumia se queda en bucle y no llega a conclusión.',
    'of Innocence': 'Tu papel es amar sin condiciones ni preferencias, y eso incluye a quien no lo merece. No es ingenuidad: es una elección sostenida, y dura.',
    'of the Trickster': 'Vendes, y hace falta que alguien venda: tu papel es legítimo mientras lo vendido sea real. El mismo talento, con la verdad estirada, se vuelve manipulación.',
    'of Caring': 'Cuidar es tu función fija, y el aviso es antiguo: cuidas de otros con facilidad y de ti mismo con dificultad. Ahí está todo tu aprendizaje.',
    'of Risks': 'Arriesgar es tu papel: sin el riesgo no aparece el sentido. Lo que no vale es el riesgo por adrenalina —tú preguntas siempre para qué.',
    'of Commitment': 'Comprometerte es tu función entera: un compromiso no se abandona. Por eso el sí importa tanto —un sí mal dado te hipoteca años.',
    'of Fates': 'Tu deseo acaba construyendo tu destino: lo que anhelas de verdad termina ocurriendo, para bien y para mal. Es un papel intenso.',
    'of Influence': 'Influir es tu papel fijo, con una condición: solo funciona si hay quien quiera seguirte. Una influencia tomada por la fuerza no es influencia.',
    'of Conservation': 'Proteges lo que merece sobrevivir: eres prudente y necesario. Tu sombra es el miedo al cambio disfrazado de sensatez.',
    'of Retreat': 'Retirarte es una función, no una huida: tu material se cocina en el silencio, y lo que sale de ahí es lo que tienes para contar.',
    'of Power': 'Tu poder no se explica, se usa: es fuerza disponible. La trampa es gastarla en lo que no responde a nada.',
    'of Experience': 'Experimentar la vida es tu papel: la aportación llega después, cuando cuentas lo que has vivido en muchos sitios. Sin ritmo, la experiencia se vuelve fuga.',
    'of Crisis': 'Para ti la crisis es el terreno, no la excepción: tu papel es atravesarlas sin dramatizarlas, y no meterte en la siguiente antes de digerir la anterior.',
    'of Bargains': 'El pacto es tu función: dar y recibir en términos claros. Y su regla es dura —un acuerdo que aceptas sin querer se acaba cobrando.',
    'of Opposition': 'Oponerte es tu papel, y no es negatividad: sin alguien que plante cara, muchas cosas malas pasan sin ruido. La condición es que la causa exista de verdad.',
    'of Provocation': 'Provocar es destapar: una pregunta incómoda y, de pronto, sale lo que se callaba. Útil cuando buscas algo; cruel cuando solo buscas reacción.',
    'of Denial': 'Negarte es una capacidad, no un defecto: decir que no y retirarte a recuperar. Se rompe cuando la retirada se convierte en aislamiento permanente.',
    'of Fantasy': 'Tu fantasía es el arranque de todo lo que después existe: eres imaginativo. Su límite es real —imaginas mucho más de lo que una vida puede contener.',
    'of Completion': 'Completar es tu papel entero: poca gente termina, tú sí. Tu desgaste llega cuando te metes en ciclos que nunca debieron empezar.',
    'of Insight': 'Tu intuición llega entera y sin explicación: tu papel es sostenerla hasta encontrar cómo decirla. Soltarla en bruto suele sonar a despropósito.',
    'of Alertness': 'Estar alerta es tu función: detectas el patrón que ya salió mal antes. Es un instinto, no un razonamiento, y funciona mejor cuando no lo argumentas.',
    'of Possession': 'Tu papel es hacerte cargo de lo común, no acaparar: custodias los recursos del grupo. Y la línea con la apropiación es fina.',
    'of Serendipity': 'Estás donde hay que estar sin haberlo planeado: es un papel afortunado, con una condición física —cuidar tu cuerpo es aquí cuidar tu suerte.',
    'of Oppression': 'Sientes que no encuentras la salida mientras procesas: tu papel es aguantar esa sensación sin forzar la conclusión. La comprensión llega sola, y llega tarde.',
    'of Depth': 'La profundidad es tu papel, y viene con su miedo pegado: la sensación de no saber bastante. Ese miedo es tu motor, no una señal de que sea cierto.',
    'of Principles': 'Tener principios es en ti una función social: alguien tiene que marcar dónde está la línea. Tu filo es la rigidez —un principio que no se revisa nunca se vuelve dogma.',
    'of Values': 'Custodiar los valores es tu papel fijo: no es moralismo, es notar cuándo un grupo se está saltando lo que lo mantenía en pie, y decirlo.',
    'of Shock': 'Tu papel es el sobresalto que saca a alguien del piloto automático: sueles provocarlo sin querer, solo por cómo entras.',
    'of Stillness': 'La quietud es en ti una función activa, no pasividad: te quedas el tiempo suficiente para que se vea lo que en movimiento no se ve. Cuesta en un entorno que premia la prisa.',
    'of Beginnings': 'Empezar es tu papel, y no incluye terminar. Entenderlo te quita mucha culpa: tu aportación es el arranque, y otros pueden llevarlo al final.',
    'of Ambition': 'La ambición es tu función, con una regla: prospera cuando se la reconoce. Empujar sin alianza te deja subiendo solo una escalera muy larga.',
    'of Moods': 'Tus estados de ánimo son tu terreno de trabajo, no un problema a resolver: tu papel es vivirlos con honestidad. La única regla práctica es no decidir desde el punto bajo.',
    'of Stimulation': 'Tu papel es el del narrador: mantener despierta a la gente con lo que cuentas. Tu límite es real —sin él, el estímulo se vuelve ruido.',
    'of Intuition': 'Tu intuición es tu instrumento entero: funciona en el presente y una sola vez, y razonarla después la borra. Tu papel es fiarte de ese primer aviso.',
    'of Vitality': 'La vitalidad es tu papel, y no es optimismo: es una energía física que te empuja a mejorar las cosas porque vivir da gusto. Cuando desaparece, algo se está corrigiendo de más.',
    'of Strategy': 'Lo tuyo es saber acercarte: cómo traspasas la distancia con otra persona. Es un papel íntimo y muy concreto —forzarlo cierra justo lo que buscaba abrir.',
    'juxtaposition:of Limitation': 'El límite es tu función, y es buena noticia: sin marco no hay forma. Su sombra es la melancolía de lo que no cabe —real, y no una razón para forzar el marco.',
    'of Thinking': 'Pensar es en ti una presión, no un pasatiempo: te empuja a saber sin garantizarte respuesta. Tu papel es sostener la pregunta sin fabricar conclusiones para calmarla.',
    'of Detail': 'Tu papel es poner nombre a las cosas para que se puedan manejar: una aportación enorme y poco reconocida. Tu sombra es perderte en lo pequeño y no ver el conjunto.',
    'of Doubts': 'Dudar es tu función, y es valiosa: alguien tiene que preguntar si esto funciona de verdad. Dirigida hacia dentro paraliza; dirigida hacia fuera protege al grupo.',
    'of Confusion': 'Para ti la confusión es el estado normal de trabajo, no un fallo: recibes más material del que puedes ordenar, y el orden llega solo, con el tiempo. Forzarlo es lo único que lo empeora.'
  },
