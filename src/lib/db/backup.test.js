...
describe('cookie-vault encoding', () => {
  it('roundtrips the saved list, cookie-safe, order preserved, ids dropped', async () => {
    const charts = [
      { id: 7, name: 'Autor', createdAt: '2026-07-01T10:00:00.000Z', sortOrder: 3, birth, type: 'manifestor' },
      { id: 2, name: 'Otra carta', createdAt: '2026-07-02T10:00:00.000Z', sortOrder: 9, birth: { ...birth, date: '1984-01-30', time: '01:00' } }
    ];
    const payload = await encodeCharts(charts);
    expect(payload).toMatch(/^[A-Za-z0-9._-]+$/);

    const back = await decodeCharts(payload);
    expect(back).toHaveLength(2);
    expect(back[0]).toMatchObject({ name: 'Autor', sortOrder: 0, type: 'manifestor', birth });
  ...
  it('roundtrips an empty list', async () => {
    expect(await decodeCharts(await encodeCharts([]))).toEqual([]);
  });
});
