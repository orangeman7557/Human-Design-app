...
- **Phase L — Launch (→ 1.0).** 🟡 Pre-launch hardening pass that gates the 1.0
  ...
  point at `https://hdchart.app`. Pending minor: `www → root` redirect at Cloudflare.
  ...
  **Step 5 (support/donations) done 2026-07-03** — the About modal gets a support
  row: a "send love" heart (colour-cycling pop + confetti via the Web Animations
  API, no libraries, plus a global click counter served by the new `/api/love`
  ...
  **Buy Me a Coffee** (author's account; replaces the original Ko-fi plan).
  Pending to activate the counter in production: create the KV namespace in the
  Cloudflare dashboard and paste its id in `wrangler.jsonc` (binding left
  commented out so deploys keep working meanwhile).
  Only the `www` redirect (minor) and the 1.0.0 bump (step 7) are left.
