...
Last updated: 2026-08-10 (auditoría completa + verificación en vivo + Fase 11 creada).
...
> - **HSTS** añadido en los dos caminos (`_headers` + Worker), `max-age=15552000; includeSubDomains`, **sin `preload`** a propósito — el preload es prácticamente irreversible y merece decisión aparte. **OJO: esto no sustituye al toggle «Always Use HTTPS» de Cloudflare**, que sigue pendiente y es la corrección autoritativa (hoy `http://hdchart.app/en` responde 200 en claro).
