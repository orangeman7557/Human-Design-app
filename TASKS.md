...
Last updated: 2026-08-09 (v1.5.3 + retoques de texto; diseño de etiquetas aprobado).

> Latest change (2026-08-09, **dos retoques de texto + diseño aprobado de etiquetas para cartas guardadas**): los retoques de texto mergeados a `staging` (sin bump; **`main` pendiente**). La función de etiquetas está en fase de **diseño aprobado (maqueta v2)**, implementación pendiente.
> **Dos retoques de texto (en código).** (a) En los prompts para IA, el texto previo al enlace se reduce a **«Datos de la carta: <enlace>»** (`chartLink` en `content/es.js` y `content/en.js`; en inglés «Chart data:»). (b) En la sección de la cruz del informe, «Y aquí está lo importante:» → **«Y aquí algo importante en lo que conviene insistir:»** (`crossEssence`, `content/es.js`).
> **Etiquetas + buscador de cartas guardadas (diseño, pendiente de construir).** En «Cartas guardadas»: (1) **buscador** arriba a la derecha (lupa + campo); al enfocar despliega un menú discreto con las **últimas búsquedas (máx. 3)** + divisor + las **etiquetas** (icono + nombre); pulsar escribe ese texto en el campo y filtra. Por ahora busca solo lo visible (nombre, tipo, fecha, lugar, etiquetas). (2) El botón de editar de cada chip se **parte en dos** apilados (editar arriba, **etiquetas ▼** abajo, icono de etiqueta gris); borrar sigue a la derecha a altura completa. El menú de etiquetas lista las etiquetas por defecto (Familia, Pareja, Amigos, Trabajo, Estudio) con check/resalte al asignar (orden fijo, no se cierra al marcar) y una opción **«Crear / editar etiquetas…»** que abre un modal (crear, renombrar, reordenar, eliminar). (3) Las etiquetas asignadas aparecen como **3ª línea del chip** (icono + nombres, «…» si no caben); todos los chips igualan altura con el texto centrado. Persistencia local (Dexie) — esquema por definir.
