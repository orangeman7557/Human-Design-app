...
Last updated: 2026-08-24 (versión 1.5.5 — tanda de UI, etiquetas por idioma y copia de seguridad por correo; merge a `staging` y a `main`).

> Latest change (2026-08-24, **versión 1.5.5 — tanda de ajustes de UI, etiquetas por idioma y copia de seguridad por correo**): sesión larga de retoques pedidos por el autor, construidos y verificados uno a uno en el navegador; el gesto del drawer y el envío por Gmail los verificó el autor en su móvil. 40/40 tests, build limpio.
> **Preview del enlace compartido** (`hooks.server.js`). En español el título pasa a «<nombre> · Carta de Diseño Humano» y la descripción a «…Explora esta carta de Diseño Humano en HDchart.app.» (en inglés, igual con el dominio al final). **Lo que no se puede tocar**: la tercera línea de la tarjeta de WhatsApp es el *host* que el previsualizador saca de la URL, y todo parser de URL lo pasa a minúsculas (norma WHATWG) — por eso el dominio con mayúsculas se lleva al texto de la descripción, que es donde sí lo controlamos. `og:site_name` y la imagen OG, intactos.
> **Drawer: abrir al segundo toque y cerrar arrastrando** (`chart/+page.svelte`, `ElementInfo.svelte`). En táctil el primer toque revela la «i» y **el segundo abre el drawer** — ya no hace falta apuntar a la «i»; a cambio desaparece el toque para deseleccionar (decisión consciente del autor). El drawer se **arrastra hacia abajo para cerrarlo** desde su cromo (raya, cabecera, pie de IA); el cuerpo de texto y la caja del prompt conservan su scroll. Umbral 90 px; por debajo vuelve a su sitio. La «i» y la ✕ siguen igual.
> **Copia de seguridad por correo.** Tercer botón (avión de papel) junto a descargar/importar: entrega el fichero a la hoja de compartir del sistema, donde están las apps de correo — copia por email sin fontanería de correo. **Las copias pasan a `hdchart-YYYY-MM-DD.txt`** (dentro, el mismo JSON de siempre): Chromium valida los ficheros compartidos contra **dos listas blancas, extensión y MIME**, y ni `.json` ni `application/json` están en ellas, así que un `.json` **no se puede compartir en absoluto**; `.txt` + `text/plain` (declarado exactamente así, sin `charset`) pasa en todas partes. El importador acepta `.txt` y `.json` (copias antiguas). Asunto y cuerpo viajan como título/texto del share: Gmail en Android los respeta, pero lo decide la app receptora. **Limitación de plataforma** (anotada en BACKLOG): la hoja de compartir de macOS no ofrece Mail para ficheros de texto plano, así que en Chrome de escritorio el correo no aparece entre las opciones. De paso, fuera el `"version": 2` del export, que no leía nadie.
> **Etiquetas por idioma.** Las sembradas (Familia/Family…) guardan una **clave** y siguen el idioma de la app, **en el acto** al cambiarlo (la sincronización cuelga del idioma de la ruta, no del montaje: cambiar de idioma es navegación por cliente y `onMount` no vuelve a correr). Como las cartas guardan sus etiquetas **por nombre**, traducir es un renombrado y reescribe todas las cartas que la llevan. Una etiqueta creada o **renombrada** por el usuario pierde la clave y ya no se traduce nunca. Las filas sembradas antes de existir las claves se **adoptan por nombre**, pero solo si en la tabla queda al menos la mitad del juego de un idioma, para no confundir una etiqueta escrita a mano con una por defecto.
> **Listado de cartas guardadas.** (a) Cada fila en **una sola línea**: el tipo se abrevia primero («Generador» → «Gen.», EN «Man. Gen.») y solo entonces se recorta el nombre con «…»; los anchos se miden **con canvas**, no renderizando y volviendo a medir, para que elegir la etiqueta no realimente la medida. El buscador sigue leyendo la carta, no la fila. (b) **Todas las tarjetas miden lo mismo**, con o sin etiquetas: altura fija con las dos líneas de texto centradas dentro, y la lista solo usa la altura alta mientras **alguna** carta tenga etiquetas (si ninguna la tiene, queda tan compacta como antes). En escritorio, 2 px menos de aire arriba y abajo. (c) Nombres de carta **máximo 40 caracteres**, en el formulario y en el diálogo de renombrar. (d) El **menú de etiquetas ya no se cierra al marcar** — se ponen varias seguidas y se cierra con el botón o pinchando fuera (revierte a propósito el «se cierra al asignar» del 1.5.4). (e) La lupa del buscador se centra en su campo (la caja se estiraba a la fila del título) y el campo gana 4 px de alto en escritorio.
> **Diálogos por encima de todo.** Un renombrar/borrar abierto desde el gestor de etiquetas salía **por debajo del gestor**: el `z-index` del velo abre contexto de apilamiento, así que la caja del diálogo se ordenaba dentro de ese 70 y el modal (71) le ganaba. Ahora el diálogo va por encima de cualquier overlay y su velo atenúa lo que lo abrió; en móvil, los diálogos **sin campo de texto** salen centrados y los que sí lo tienen siguen anclados arriba (el teclado tapa la mitad inferior). Escape cierra solo el diálogo, no también el gestor.
> **Dos bugs de idioma y de foco.** (a) El botón «¡Mándame amor!» guardaba **el texto** en un estado inicializado al crearse; como «acerca de» vive en el pie de la home y su instancia sobrevive al cambio de idioma, se quedaba en el idioma en que nació. Ahora guarda **qué** dice (reposo / «MÁS» / cuál agradecimiento) y las palabras se resuelven en el idioma activo. (b) Los overlays pintaban el **anillo de foco** alrededor del panel — apareció al poder abrir el drawer tocando el elemento (un `<span>`), sin nada enfocado antes. Arreglado en `focus-trap.js`, que ya ponía `tabindex="-1"` con esa misma intención.
> **Textos.** El drawer de la cruz abre con **la lectura de la cruz** (antes vivía bajo el esquema de puertas, donde nadie llega) y después van el ángulo, las cuatro puertas, el cuarto del mandala, el esquema y el handoff. La frase del mandala se aligera en ES y EN. Fuera el 🙂 de «acerca de». La hoja de compartir de la página de carta ya dice «Carta de Diseño Humano» en español.
> **Verificado en navegador**: preview OG, doble toque en los cinco tipos de elemento, gesto de arrastre, fichero de copia generado y **reimportado**, las tres reglas de las etiquetas (por defecto se traducen / creada se mantiene / renombrada nunca vuelve a traducirse) con las cartas reescritas, diálogo por delante y centrado, filas de una línea a 375 px, alturas uniformes y adaptativas, lupa centrada, botón de amor en inglés tras cambiar de idioma. Cerrado con bump a **1.5.5** y merge a `staging` **y a `main`**.
>
> Previo (2026-08-19, **los prompts «sobre esta carta» vuelven a ser específicos de la carta**): el enlace compartible (1.5.3, ago 2026) se había llevado por delante el ángulo de carta. Al añadir el enlace, `buildPrompts` **sustituía el prompt de carta por el general** y le pegaba la URL al final, así que «Sobre esta carta» e «Info general» decían literalmente lo mismo y solo se distinguían por el enlace.
> **Arreglo (`prompts.js`).** El ángulo de carta vuelve a construirse entero; lo único que cambia con enlace es el descriptor: `who` (el volcado largo — «un Manifestador, perfil 1/3, autoridad emocional (Plexo solar), definición split, centros definidos…») se sustituye por **`whoLink`**, corto, porque el enlace ya sirve esos datos → **«para un Manifestador con esta carta (ver enlace al final)»**. Todo lo demás específico de la carta **se conserva**: el estado de la puerta con sus activaciones («…la puerta 1, que en esta carta está activada por Saturno en Personalidad (línea 4) y está colgante»), el del canal, las dos activaciones del planeta, la cruz con sus cuatro puertas. Implementado pasando un flag `linked` a `buildPromptsCore`/`conceptPrompts` y curándolo en un `askC(subject)` local; sin enlace, el comportamiento es idéntico al de antes.
> **Instrucción explícita de abrir el enlace** (petición del autor: hay IAs que no siguen una URL suelta). El cierre pasa de «Datos de la carta: <url>» a **«Consulta el siguiente enlace para ver los datos de la carta: <url>»** (EN: «Please open the following link to see the chart data:»), en su propio párrafo y con la URL al final de línea. Mismo criterio en el **handoff del informe** (`closingPromptLink`): la URL deja de llevar un punto pegado detrás y el «me gustaría profundizar en…» baja a su propio párrafo, para que el usuario complete al final. Esto revierte a propósito el retoque de textos del 1.5.4 en `chartLink`.
> **Retoque del descriptor** (2ª pasada, misma sesión): «con la carta del enlace de abajo» → **«con esta carta (ver enlace al final)»** (EN: «with this chart (see the link at the end)»), a petición del autor — nombra la carta primero y el enlace como referencia, en vez de definirla por dónde está el enlace.
> **Bug de plantilla inglesa, encontrado al revisar el resultado.** `ask`/`askChart` en inglés estaban **calcadas del español** (`can you explain {subject} in detail?` ← «¿me explicas en detalle {subject}?»). Con sujetos que acaban en subordinada —los de puerta y canal— el modificador quedaba colgando: *«can you explain gate 1, which in this chart is activated by Saturn in Personality (line 4) and is hanging (…) **in detail**?»*. Movido delante (`can you explain in detail {subject}?`), la misma forma que ya usaba `planetChart`. Revisados los 17 tipos de prompt en inglés.
> **Norma registrada** (petición del autor): *el inglés es la mejor adaptación del contenido, nunca una traducción literal del español* — **y eso incluye las plantillas con huecos**, no solo la prosa, porque el orden de palabras español puede funcionar con un relleno corto y romperse con uno largo. Anotada en `CLAUDE.md` §4 (bullet «Los catálogos no son traducciones espejo») y en `docs/fase-m-multilingue.md` § «Detalles con trampa al traducir», con la regla operativa: **toda plantilla nueva se prueba con el relleno más largo que admita**.
> **Verificado**: prompts generados a mano para la carta del autor en **ES y EN** (tipo, estrategia, autoridad, definición, sin definición, centro, puerta activa/colgante/inactiva, canal, cruz, señales, planeta, bodygraph, I Ching, perfil, línea, columna de activación, informe) y el camino sin enlace intacto. **40/40 tests**, build sin avisos nuevos. Sin cambio de versión (sigue 1.5.4). Merge a `staging` y a `main`.
>
> Previo (2026-08-10, **auditoría completa, verificación en vivo y creación de la Fase 11**): sesión de revisión, **sin cambios de código todavía**. Tres bloques.
> **1. Verificación en vivo — cerrado el bloque que se arrastraba desde la 1.0.** Verificado contra producción: `www`→raíz (301) y raíz→`/en` (307); **contador KV vivo** (`{"count":194,"senders":5}`); service worker desplegado con su precache y el *fallback* offline; `_headers` no se sirve (404, correcto); robots/sitemap/manifest OK; OG por carta en ambos idiomas; **el enlace para IA funciona** (`Accept: application/json` → perfil calculado, y el HTML lleva el bloque *fallback* resuelto); navegador **sin errores de consola**, `<html lang>` correcto, JSON-LD presente, imágenes con `alt`. 40/40 tests y build limpio. **Sigue sin poder verificarse desde aquí:** iPhone real y el enlace «instalar como app» en Chrome Android — necesitan los dispositivos del autor.
> **2. Auditoría — 6 hallazgos.** **Altos:** (a) **`http://` no redirige a `https://`** — sirve 200 en claro — y **no hay HSTS** (se arregla en el panel de Cloudflare, sin código); (b) **`/chart` no lleva ninguna cabecera de seguridad** porque `_headers` solo aplica a assets estáticos y `/chart` pasa por el Worker — sin `referrer-policy`, al pulsar un enlace externo desde la carta la URL con los datos de nacimiento viaja en el `Referer`; (c) **los enlaces de carta compartida son indexables** (sin `noindex`, `robots.txt` permite todo), así que un enlace pegado en un foro puede acabar indexado con nombre y datos de nacimiento. **Medios:** (d) **la home descarga 919 KB de JS (316 KB gzip)** y el mayor chunk (707 KB / 229 KB gzip) **contiene el pack de contenido de los dos idiomas** —130 «Cruz de ángulo» y 66 «Right Angle Cross» en el fichero que carga la portada—, porque `en.js` hace deep-merge sobre `es.js`; (e) **el manifest no tiene `screenshots`**, lo que **bloquea la Fase P**; (f) `/stats` responde 200 en producción (la API sí está bien cerrada a staging — **no se filtra ningún dato**). **Bajos:** sitemap sin `lastmod`, componentes grandes, ~20 elementos con `onclick` sin ruta de teclado. **Nada que arreglar** en `api/stat` (allowlist, tope, same-origin, agregado puro) ni en la política de privacidad, que ya cubre la analítica.
> **2b. Arreglos de la auditoría — hechos y verificados en la misma sesión (2026-08-10).** Cerrados **5 de los 6** hallazgos.
> - **Cabeceras de seguridad en el Worker** (`hooks.server.js`). `static/_headers` solo cubre assets estáticos, así que todo lo que sirve el Worker —el redirect de la raíz, los legacy y sobre todo `/<lang>/chart`— salía **sin ninguna cabecera**. Nuevo `SECURITY_HEADERS` + helper `secure()` aplicado a toda respuesta de runtime (el prerender se salta, que ahí no hay petición que endurecer). Espejo exacto de `_headers`; **mantener los dos en sync**. La que más pesaba aquí es `Referrer-Policy`: la URL de una carta lleva los datos de nacimiento en la query, así que sin ella la URL entera viajaba en el `Referer` de cualquier clic saliente (handoff de IA, donaciones).
> - **HSTS** añadido en los dos caminos (`_headers` + Worker), `max-age=15552000; includeSubDomains`, **sin `preload`** a propósito — el preload es prácticamente irreversible y merece decisión aparte. La corrección autoritativa era el toggle **«Always Use HTTPS» de Cloudflare**, que **el autor activó el 2026-08-10** y quedó **verificado en vivo**: `http://hdchart.app/` y `http://hdchart.app/en` devuelven ahora **301 a https** (antes servían 200 en claro). La cabecera HSTS aparecerá en producción con el despliegue de este cambio.
> - **`X-Robots-Tag: noindex, nofollow` en `/<lang>/chart`** (las dos ramas, HTML y JSON). Un enlace compartido pegado en un foro era indexable con nombre y datos de nacimiento. Las *previews* sociales no se ven afectadas: los unfurlers no obedecen a robots.
> - **`/stats` cerrado en producción** (404 fuera de hosts staging, mismo test que `api/stats` y `robots.txt`). Los datos ya estaban bien cerrados; esto cierra la página, que anunciaba que existe un panel. Verificado que `staging.hdchart.app` y `…-staging.workers.dev` siguen pasando.
> - **Peso de la home: 919 KB → 439 KB raw, 316 → 166 KB gzip (mitad).** Causa: la home importaba `getDisplayLabels` desde `content/index.js`, que importa estáticamente `es.js` **y** `en.js` (~526 KB de prosa: las 192 cruces, las 64 puertas, el informe entero) **para leer cinco nombres de tipo**; y `WhatIsHD.svelte` arrastraba el pack por el mismo camino. Arreglo en dos partes: (a) el bloque `labels` sale a `content/labels-{es,en}.js` y los packs lo **reimportan**, así `pack(lang).labels` y el test de paridad quedan **idénticos**, más un accesor ligero `content/labels.js` que solo importa esos dos módulos; (b) `WhatIsHD` carga `content/index.js` con **import dinámico al abrir el modal** (la home es prerenderizada, así que el import solo ocurre en un clic real). Verificado en navegador: cruces fuera del bundle eager, etiquetas de tipo correctas en los chips, modal que abre con sus 7 párrafos y las hormigas, y la carta en inglés dando «Right Angle Cross of Eden» con 64 puertas y consola limpia.
> - **Menores**: `<lastmod>` en las 4 URLs del sitemap; `categories` en el manifest.
> - **NO cerrado — `screenshots` del manifest**: son assets de imagen y elegirlos es decisión de diseño del autor. Especificación exacta anotada en BACKLOG § Fase 11 / Nivel 4. **Sigue bloqueando la Fase P.**
> - **Deliberadamente NO tocado**: la deuda de accesibilidad de teclado (~20 elementos con `onclick` sin ruta de teclado) — es un cambio del modelo de interacción de la página de carta, ya registrado en la auditoría de 2026-06-15, y no cabía en una tanda de seguridad sin arriesgar el hover/pin. Igual con el tamaño de los componentes. Los dos avisos de *unused CSS selector* de `chart/+page.svelte` son **preexistentes** (ese fichero no se tocó).
>
> **3. Fase 11 — Distribución y marketing (creada).** El diagnóstico: la app **tiene 4 URLs indexables** mientras sus packs guardan una enciclopedia bilingüe ya escrita (~400-600 páginas de contenido único) que Google no ve, y los competidores que rankean lo hacen con ese mismo patrón. Cinco niveles en `CLAUDE.md` §6 y detallados en `BACKLOG.md` § "Fase 11". **Decisión del autor: se arranca por seguridad** (los tres hallazgos altos), no por marketing.
>
> Previo (2026-08-10, **versión 1.5.4 — etiquetas y buscador de cartas guardadas, y dos retoques de texto**): construido y **verificado en el navegador** (asignar/renombrar/eliminar reflejan en las cartas, filtrado por nombre/tipo/lugar/etiqueta insensible a acentos, recientes, semilla de etiquetas por defecto). 40/40 tests, build sin avisos. Cerrado con bump a **1.5.4** y merge a `staging` **y a `main`**.
> **Dos retoques de texto.** (a) En los prompts para IA, el texto previo al enlace se reduce a **«Datos de la carta: <enlace>»** (`chartLink` en `content/es.js` y `content/en.js`; en inglés «Chart data:»). (b) En la sección de la cruz del informe, «Y aquí está lo importante:» → **«Y aquí algo importante en lo que conviene insistir:»** (`crossEssence`, `content/es.js`).
> **Etiquetas + buscador (implementado).** En «Cartas guardadas»: (1) **buscador** arriba a la derecha; al enfocar/clicar despliega un menú discreto con **últimas búsquedas (máx. 3, en `localStorage`)** + divisor + las **etiquetas** (icono + nombre); pulsar escribe ese texto en el campo y filtra. Busca solo lo visible (nombre, tipo, fecha, lugar, etiquetas); mientras se filtra, el drag & drop se desactiva. (2) El botón de editar de cada chip se **parte en dos** apilados (editar arriba, **etiquetas ▾** abajo, icono de etiqueta gris); borrar a la derecha a altura completa. El menú de etiquetas lista todas con check + resalte oro al asignar (**orden fijo, no se cierra al marcar**) y **«Crear / editar etiquetas…»** que abre un modal (crear, renombrar, reordenar por drag, eliminar). (3) Las etiquetas asignadas van como **3ª línea del chip** (icono + nombres en orden global, «…» si no caben); todos los chips igualan altura (mín. 3,5rem, texto centrado) — más ceñidos a petición del autor.
> **Datos (Dexie v3).** Nueva tabla `labels` (`{id, name, sortOrder}`) — **lista global** editable; cada carta guarda sus etiquetas **por nombre** (`chart.labels: string[]`), así que renombrar reescribe el nombre en todas las cartas y el valor sobrevive al backup de la bóveda-cookie y al export/import sin remapear ids. Backup (`encodeCharts`/`decodeCharts` → forma `{c, l}`, con retrocompat de la forma antigua de array), `exportCharts`/`importCharts` (v2 del fichero, acepta v1) y la restauración de la bóveda incluyen ahora las etiquetas. Semilla de etiquetas por defecto **localizada y una sola vez** desde la UI (`seedDefaultLabels` + flag `hd:labels-seeded`), no en Dexie. Nuevo componente `LabelManager.svelte` para el modal.
> **Tanda de ajustes 2 (misma sesión, verificados).** Buscador: **aún más bajo** (≈35% en escritorio, ≈15% en móvil; se anula el `height:2.75rem` global de los campos) y **sin placeholder** (basta la lupa); su **«x»** ahora **quita el foco** y cierra el desplegable (se ven todas las cartas). Desplegable del buscador: la **«x» de recientes pasa a cada búsqueda** (borrado individual); **sección «Tipos»** reordenada a **G, GM, P, M, R**; el **engranaje** va **junto al título «Etiquetas»** (no al extremo). Menú de asignar: **sin check** —las seleccionadas se marcan con el **icono de etiqueta relleno y en naranja**, alineadas a la izquierda—, **se cierra al asignar**, y la **casilla «Nueva etiqueta…»** es más baja y **alineada a la izquierda** en móvil. El **botón del chip** usa el **icono relleno** si la carta ya tiene etiquetas. **Las etiquetas del chip son pinchables** (hover naranja en escritorio, subrayado en táctil) y **rellenan el buscador** con esa etiqueta sin abrir el desplegable. Modal de gestión: botón **«+»** (antes «Añadir») y **«Cerrar»** al pie. About: «…puedes hacerlo aquí 🙂» y «Ojalá que la app te sea útil.» + salto de línea + «¡Y que vivas feliz…!» (ES y EN). Modal «saber más» **reescrito** (3 párrafos, cookie técnica, y en negrita el aviso de que limpiar datos las borra); tooltip **«Descargar cartas»**.
> **Tanda de ajustes (misma sesión, verificados).** Buscador **más bajo y ~30% más estrecho** (a la altura del título) con **«x» para limpiar** (patrón del campo de ciudad); búsqueda **insensible a acentos** («alva» encuentra «Álvaro», vía `normalize('NFD')`); **el buscador ya no se mueve al filtrar** (se reserva la altura de la lista en `.list-wrap` para que el bloque centrado no se recentre). Desplegable del buscador: sección **«Tipos»** (los 5, icono de persona) tras «Etiquetas»; **«x» discreta** en «Recientes» para vaciarlas; **engranaje** (⚙) a la derecha de «Etiquetas» que abre el gestor; los **recientes ya no incluyen** etiquetas ni tipos (solo texto tecleado + Enter). Menú de asignar: **título «Etiquetas» + engranaje** arriba, **icono de etiqueta junto a cada una**, **casilla «Nueva etiqueta…»** al final (crea y asigna), **se cierra al asignar** (reabrir para más) y **sin la flechita** del botón. Modal de gestión con **botón «Cerrar»** al pie. Home: **«¿Qué es el Diseño Humano?»** pasa a color atenuado (como el subtítulo), con el hover naranja intacto. Modal «saber más»: texto **acortado** (cookie técnica + qué las borra); **sin** frase de máximo de cartas — se midió y la copia admite **300+** (muy por encima de 100), así que no aplica el aviso.

> Previo (2026-08-09, **versión 1.5.3 — enlace compartible para IA (embed + hora) y resúmenes de las 112 cruces**): validado en vivo por el autor (ChatGPT lee el enlace, hora correcta, textos de cruces revisados) y cerrado con bump a **1.5.3** y merge a `staging` **y a `main`**.
> **Enlace de perfil para IA — sin detección, por embed.** La detección por cabeceras no era fiable (ChatGPT pasa por infra que reescribe `Sec-Fetch`/UA). Solución final: el Worker **incrusta el perfil en el HTML** (texto legible + JSON); un cliente con JS lo borra al instante, uno sin JS lo lee. Extra: `Accept: application/json` → JSON puro. Lo leen Claude y ChatGPT.
> **Bug de la hora del enlace — arreglado.** El `t=HHMM` viajaba como parámetro suelto y algún limpiador de URLs externo lo despojaba → la hora caía a 00:00 y **toda la carta salía mal**. Se pliega la hora dentro de `d` (`YYYYMMDD` → `YYYYMMDDHHMM`); enlaces antiguos con `t` siguen decodificando. De paso, la URL se simplifica un parámetro.
> **Resúmenes de las 112 cruces (informe).** El informe inserta un párrafo breve en 2ª persona de la cruz concreta entre el ángulo y el cierre (bloque `crossSummary`, `getCrossSummary`), en ES y EN, derivado de `crossEssence`.
> **Nota:** el fallo inicial de acceso de ChatGPT no llegó a diagnosticarse (probable caché/respuesta antigua antes del embed); el estado final es estable. Los diagnósticos temporales (`/ai-test`, marcador `AI_CHART_TEST`) se retiraron.
...
> **Enlace de perfil compartible (backlog).** El mismo enlace de "compartir" (`/{lang}/chart?…`) hace legible el perfil calculado a una IA que lo sigue desde un prompt. **Sin detección de cliente** (la detección por cabeceras falló: ChatGPT pasa por infra que reescribe `Sec-Fetch`/UA y no recibía nada). En su lugar, el Worker (`hooks.server.js`) **incrusta el perfil en el HTML** como *fallback* (texto legible + JSON completo): un cliente con JS lo borra al instante con un `<script>` inline (antes de pintar), uno sin JS lo lee. Extra: `Accept: application/json` devuelve JSON puro. El JSON lleva nacimiento + perfil calculado con clave técnica **y** etiqueta legible (`src/lib/hd/profile-json.js`). **Consecuencia:** la carta se calcula en cada vista del enlace (no solo en bots) — coste asumido, medición pendiente en BACKLOG. Los prompts "sobre esta carta" y el del informe **ya no llevan los datos en el texto**: apuntan al enlace.
...
> **Tanda 2 de retoques.** Título de la tabla de cuartos ("Los 4 cuartos que agrupan las 192 cruces posibles", con "cruces" enlazado); **todos los drawers con prompt** (faltaban cuartos e I Ching); "Vivir tu diseño" pasa de dorado a **negrita** + **scroll-spy** en el índice del informe; más aire tras la sub-viñeta de "desalineamiento"; la cruz del informe encabeza con **"Tu cruz es: <nombre> (<puertas>)"**. **Pendiente (BACKLOG):** el párrafo resumen por cruz (~112 × 2 idiomas) — pasada de contenido aparte; la estructura ya está.
...
> **Bug (Android): el informe se escapaba por arriba y no se podía cerrar.** El overlay del informe era `height: 92vh` anclado a `bottom:0`; en móvil `vh` es el viewport *grande* (barra de direcciones oculta), así que el 8 % superior —cabecera y ✕— quedaba **por encima del área visible**. Cambiado a `dvh` (con *fallback* `vh`) en `InitialReport.svelte` (92/88) y, por el mismo patrón, en el bottom sheet de `ElementInfo.svelte` (`max-height:88dvh`). No reproducible en el preview (viewport fijo, sin barra dinámica); **confirmado arreglado por el autor en Android real (2026-07-23)**.
> **Cruz «of Limitation» duplicada — resuelta.** El build avisaba de una **clave duplicada** en `crossEssence`: son **dos cruces distintas con el mismo nombre desnudo** —Ángulo izquierdo (32/42\|56/60) y Yuxtaposición (60/56\|28/27)—, así que la segunda pisaba a la primera y una interpretación no se mostraba nunca. Las claves pasan a estar **cualificadas por ángulo** (`left:of Limitation`, `juxtaposition:of Limitation`) y `getCrossEssence` prueba `"<ángulo>:<nombre>"` antes que el nombre desnudo; el resto (nombres únicos) sigue igual. Test actualizado para aceptar las claves cualificadas. Verificado: cada cruz muestra ahora **su propia** esencia (izq → 32/42, yux → 60/56) y el build ya no avisa.
> **Home — enlace «¿Qué es el Diseño Humano?».** Pegado al subtítulo (margen 0,55→0,15rem, parece un salto de línea, no de párrafo) y **resalta con cambio de color** en hover (a naranja) en vez de subrayado.
> **Modal «¿Qué es…?» — recortado y afinado.** Se quita el título intermedio «La analogía de las hormigas» (las hormigas fluyen como prosa, igual que en el informe), se **elimina la sección del bodygraph**, y el título «El Diseño Humano como experimento vital» va **en naranja** (`.body p.subhead`, para ganar a `.body p` por especificidad). En la invitación final, «de arriba» fuera y **«Rellena el formulario» es ahora un enlace** que cierra el modal (devuelve al usuario a la home). Cadenas `whatHd.invite` → `inviteLink` + `inviteRest` en los dos idiomas.
> **Home móvil — el subtítulo cabía en dos líneas.** «registro» no entraba por 1px (la línea a 0,9rem pide 376px y el móvil da 375). Se **ensancha la tagline** más allá de los campos (`margin-inline:-1rem`, ~12px de margen a cada lado, no cero) y se **baja la fuente a 0,8rem** solo en móvil; medido: cabe en **una línea** con 10px de holgura a 375px.
> **Verificado por DOM/medición** (el modal-no-cierra del preview era un artefacto: `document.visibilityState:"hidden"` congela las animaciones WAAPI y la transición `fly` de salida no completa, así que Svelte no desmonta — le pasa igual a About/StorageInfo; en un navegador real cierra normal). **39/39 tests, build sin errores ni avisos** (el «Duplicate key "of Limitation"» ya no aparece).
> **BACKLOG**: anotada la mejora de **scroll-spy del índice del informe** (resaltar el chip del índice de la sección visible al bajar).
> **Cierre**: bump a **1.5.1** (`package.json`, que alimenta `__APP_VERSION__`) y merge a `staging` y a `main`.
>
> Previo (2026-07-23, **ajustes de drawers + punto de entrada «¿Qué es el Diseño Humano?»**): cuatro retoques sobre la 1.5.0, sin bump de versión.
> **Índice de las 192 cruces — el nombre es un chip.** En el drawer general de la cruz, cada fila del índice muestra el nombre de la cruz como **chip clicable** que abre el drawer de esa cruz concreta (con su interpretación propia), no solo el chip de la puerta del Sol. El chip lleva **solo el nombre**; las cuatro puertas quedan como texto al lado. Nuevo `getCrossInfoByKey('<puertaSol>|<ángulo>')` que regenera el cuarteto desde la rueda (`crossQuartet`) y comparte el ensamblador (`buildCrossInfo`) con `getCrossInfo(chart)`; el despachador de la página de carta enruta la clave con `|` a esa función.
> **Drawers de escritorio — se maximiza el texto.** El panel pasa a ser una columna flex acotada (`overflow:hidden` + body `flex:1`): «Saber más usando IA» queda **anclada abajo del todo** y el área de texto **crece hasta ella**, en vez de cortarse en un alto fijo (se retira el `max-height` del body y el empujón manual de 3,8rem del `sep`). Al abrir «Ver/editar el prompt» el texto se **comprime hacia arriba** (el body encoge y hace scroll) y el textarea se acota (`max-height:40vh` con scroll propio), así nada desborda. Lo mismo en **móvil**: el body apura el espacio antes de la sección IA. Además, **el drawer de escritorio es ensanchable** arrastrando el borde izquierdo, y el ancho **se recuerda** (localStorage `hd:drawer-width`, 340–820px).
> **Chips de los facts, menos tabulación.** En los drawers de centro (canales/puertas) y de canal (centros/puertas) los chips quedaban muy separados de su etiqueta (columna fija de 6,6rem). Ahora la etiqueta va **pegada a los chips** (gap de 8px, ~13% de la tabulación anterior); solo la **cruz** conserva la columna ancha alineada (flag `factsAlign` → clase `.facts.aligned`) porque necesita que «sol/tierra» caigan en la misma vertical entre Personalidad y Diseño.
> **Home — «¿Qué es el Diseño Humano?».** Enlace discreto y centrado bajo el subtítulo (misma estética que «Borrar formulario» pero en el **blanco del título**, con subrayado al hover) que abre un modal sencillo (`WhatIsHD.svelte`) con la **Parte A del informe** (qué es HD + hormigas + experimento vital + bodygraph) más un párrafo que **invita a rellenar el formulario**. Atado al idioma de la ruta (prerender-safe, `t(k,p,lang)` + `getReportSection(id,lang)`), con los enlaces internos del informe convertidos a **texto plano** (no hay drawers en la home). El modal se centra con `inset:0 + margin:auto` (no con `transform`, que chocaba con la transición `fly`). Cierra el **ítem 3** del análisis funcional del BACKLOG.
> **Verificado en navegador** (ES y EN, escritorio y móvil): el chip de nombre abre la cruz concreta y su drawer propio; IA anclada abajo con el texto llenando el espacio; redimensionado que **persiste tras recargar** (600px); «ver prompt» que comprime el texto en vez de desbordar; chips de centro/canal pegados a la etiqueta y la cruz alineada; modal de la home con la Parte A y el párrafo de invitación, enlaces en texto plano; el modal es opaco y está centrado (la translucidez de las capturas es artefacto del preview — el mismo aspecto que el modal «saber más» ya en producción). **39/39 tests, build sin errores ni avisos nuevos** (los dos «Duplicate key "of Limitation"» de `crossEssence` en `en.js` son previos, no de esta tanda).
> **Siguiente**: merge a staging (hecho en esta tanda).
>
> Previo (2026-07-22, **versión 1.5.0 — señales y cruz de encarnación**): cerrada la tanda y bump a **1.5.0**, con merge a `staging` y a `main`.
