...
> Latest change (2026-08-09, **enlace robusto (embed + hora) y resúmenes de las 112 cruces**): mergeado a `staging`; **`main` sigue pendiente** de que el autor confirme en vivo el enlace con la IA.
...
> **Acceso de IA sin detección + diagnóstico.** Ruta temporal `/ai-test` (HTML pelado, sin framework ni JS) para aislar si el navegador de una IA (ChatGPT) puede leer HTML del Worker. Claude lee el embed; ChatGPT (a fecha de hoy) no — pendiente del test `/ai-test`.
> **Resúmenes de las 112 cruces (informe).** El informe ya inserta un párrafo breve en 2ª persona de la cruz concreta entre el ángulo y el cierre (bloque `crossSummary`, `getCrossSummary`), en ES y EN, derivado de `crossEssence`. Cierra el pendiente que había en BACKLOG.
