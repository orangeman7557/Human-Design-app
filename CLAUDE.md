...
- **Señales + cruz de encarnación (2026-07-22):** los dos datos que faltaban frente a las apps de referencia. Las **señales** (alineamiento/desalineamiento) son una tarjeta con dos valores compuesta desde el tipo — se llaman así a propósito, y «firma»/«tema del no-yo» solo se menciona dentro del drawer. La **cruz de encarnación** (`chart.cross`) muestra su **nombre canónico** + las cuatro puertas Sol/Tierra y compone su significado desde las esencias de esas puertas. Los **192 nombres** (bloque `crossName`, clave `"<puertaSol>|<ángulo>"`) se obtuvieron de material publicado y se **validan contra la efeméride** en `cross-names.test.js`: una cruz queda determinada por (puerta del Sol de Personalidad, ángulo), así que el motor regenera el mapa completo por su cuenta y ninguna fila mal transcrita puede pasar. El informe estrena sección **«Tu propósito»**, colocada tras «Vivir tu diseño» y subordinada a ella a propósito.
