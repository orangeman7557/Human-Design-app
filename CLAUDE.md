...
- **Fase 5 — Compartir imagen** (implementada): botones compartir/descargar en la página de carta que capturan toda la vista (cabecera + resumen + bodygraph + listas) a PNG vía `html-to-image`; share sheet nativo en táctil, descarga en escritorio. **Cierra el MVP cuando se resuelvan los bugs anotados en BACKLOG ("Known bugs")**: cálculo erróneo grave (caso 1984-01-30 da Projector en vez de Reflector), descarga de imagen rota en escritorio y tipografía de cabecera. Después, estabilización: tests contra cartas reales, bug-fixing, opcional TWA para Google Play.
